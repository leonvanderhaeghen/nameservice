...
func GetCmdCreateName(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			argsPriceInt, _ := strconv.ParseInt(argsPrice, 10, 64)
			msg := types.NewMsgCreateName(cliCtx.GetFromAddress(), string(argsValue), sdk.Coins{sdk.NewInt64Coin("nametoken", argsPriceInt)})
...
func GetCmdSetName(cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			inBuf := bufio.NewReader(cmd.InOrStdin())
			txBldr := auth.NewTxBuilderFromCLI(inBuf).WithTxEncoder(utils.GetTxEncoder(cdc))
			msg := types.NewMsgSetName(cliCtx.GetFromAddress(), id, string(argsValue), sdk.Coins{sdk.NewInt64Coin("nametoken", argsPriceInt)})
			err := msg.ValidateBasic()
			if err != nil {
				return err
			}
